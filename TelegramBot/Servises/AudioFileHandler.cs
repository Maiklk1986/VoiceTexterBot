    ...
    internal class AudioFileHandler: IFileHandler
    {
            ...
            public async Task Download(string fileId, CancellationToken ct)
            {
                ...
                using (FileStream destinationStream = File.Create(inputAudioFilePath))
                {
                    ...
                    // Скачиваем файл
                    await _telegramBotClient.DownloadFile(file.FilePath, destinationStream, ct); //в образце  await _telegramBotClient.DownloadFileAsync(file.FilePath, destinationStream, ct);
            }
            }
            ...
            AudioFileConverter.TryConvert(inputAudioPath, outputAudioPath);
            ...
            Console.WriteLine("УРА!!!!" + speechText);
